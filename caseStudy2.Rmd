...
ds_full <- subset(ds_full, select = -c(Event.Clearance.Description,Event.Clearance.Group, spc_time, mminute, ss, AM.PM,Event.Clearance.Date, MM.DD.YY))
...
sample_rows = sample(1:nrow(ds_full), floor(nrow(ds_full) * fraction))
ds_full2 = ds_full[sample_rows,]

train <- ds_full2[1:round(0.85*nrow(ds_full2)), ]
test <- ds_full2[round(0.85*nrow(ds_full2)+1):nrow(ds_full2), -1]
y_real <- as.vector(ds_full2[round(0.85*nrow(ds_full2)+1):nrow(ds_full2), 1])
